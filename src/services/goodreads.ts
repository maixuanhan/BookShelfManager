import { GOODREADS_API_KEY } from 'react-native-dotenv';
    ...
    if (res.ok) {
        ...
        if (books) {
            for (let i = 0; i < Math.min(10, books.length); ++i) {
                ...
                const author = b.getElementsByTagName('author')[0]?.
                    getElementsByTagName('name')[0]?.firstChild?.nodeValue || undefined;
